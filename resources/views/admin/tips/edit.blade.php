...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tip.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tips.update", [$tip->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $tip->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $tip->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('countries') ? 'is-invalid' : '' }}" name="countries[]" id="countries" multiple>
                    @foreach($countries as $id => $country)
                        <option value="{{ $id }}" {{ (in_array($id, old('countries', [])) || $tip->countries->contains($id)) ? 'selected' : '' }}>{{ $country }}</option>
                    @endforeach
                </select>
