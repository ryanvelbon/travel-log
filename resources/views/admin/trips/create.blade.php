    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.trips.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Trip::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'planning') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('starts_at') ? 'is-invalid' : '' }}" type="text" name="starts_at" id="starts_at" value="{{ old('starts_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('ends_at') ? 'is-invalid' : '' }}" type="text" name="ends_at" id="ends_at" value="{{ old('ends_at') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('arrived_by') ? 'is-invalid' : '' }}" name="arrived_by" id="arrived_by">
                    ...
                    @foreach(App\Models\Trip::ARRIVED_BY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('arrived_by', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('purpose') ? 'is-invalid' : '' }}" name="purpose" id="purpose">
                    ...
                    @foreach(App\Models\Trip::PURPOSE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('purpose', 'vacation') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
