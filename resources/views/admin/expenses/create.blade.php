    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.expenses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_eur') ? 'is-invalid' : '' }}" type="number" name="amount_eur" id="amount_eur" value="{{ old('amount_eur', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category" required>
                    ...
                    @foreach(App\Models\Expense::CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('category', 'other') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
